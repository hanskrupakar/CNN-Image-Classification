...
if(os.path.exists('save_hist') and os.path.exists('out_hist') and os.path.exists('test_save_hist') and os.path.exists('test_out_hist')):
    ...
    X = sio.loadmat('../Datasets/features/global/MIT_8_Global_Col_Hist_24.mat')
...
inp = np.reshape(inp,(np.shape(inp)[0],12,2))
inp_test = np.reshape(inp_test,(np.shape(inp_test)[0],12,2))

x = tf.placeholder(tf.float32, [None,12,2])
y = tf.placeholder(tf.float32, [None,8])

W = tf.Variable(tf.random_normal([50,8], stddev=0.01))
W_in = tf.Variable(tf.random_normal([2,50], stddev=0.01))
B_in = tf.Variable(tf.zeros([50]))
B = tf.Variable(tf.zeros([8]))

def RNN(x, weight, bias):
    
    if(flag==1): 
        x = tf.reshape(tf.matmul(tf.reshape(x,[np.shape(inp)[0]*12,2]),W_in),[int(np.shape(inp)[0]),12,50])+B_in
    else:
        x = tf.reshape(tf.matmul(tf.reshape(x,[np.shape(inp_test)[0]*12,2]),W_in),[int(np.shape(inp_test)[0]),12,50])+B_in
    
    x = tf.transpose(x, [1, 0, 2])
    x = tf.reshape(x, [-1, 50])
    x = tf.split(0, 12, x)

    lstm = rnn.rnn_cell.BasicLSTMCell(50,forget_bias=1.0,state_is_tuple=True)
