...
if(os.path.exists('save_lstm') and os.path.exists('out_lstm') and os.path.exists('test_save_lstm') and os.path.exists('test_out_lstm')):
    with open('save_lstm','r') as f1:
        with open('out_lstm','r') as f2:
                if(f1 and f2):
                    f1.seek(0)
                    f2.seek(0)
                    inp=np.load(f1)
    ...
    with open('test_save_lstm','r') as f1:
        with open('test_out_lstm','r') as f2:
                if(f1 and f2):
                    f1.seek(0)
                    f2.seek(0)
                    inp_test=np.load(f1)
    ...
    with open('save_lstm','w+') as s:
        np.save(s,inp)
    with open('out_lstm','w+') as s:
        np.save(s,out)
    
    with open('test_save_lstm','w+') as s:
        np.save(s,inp_test)
    with open('test_out_lstm','w+') as s:
        np.save(s,out_test)
